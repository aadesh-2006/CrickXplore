...
interface MomentsTeaserProps {
  onEnterMoments?: () => void;
}

export const MomentsTeaser: React.FC<MomentsTeaserProps> = ({ onEnterMoments }) => {
  return (
    <section className="py-24 px-6 sm:px-8 relative overflow-hidden border-t border-white/[0.08]">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10 space-y-6">
        ...
        <div className="pt-4 flex flex-col items-center justify-center gap-3">
          <button
            onClick={onEnterMoments}
            className="group px-7 py-3.5 rounded-full bg-gradient-to-r from-rose-500/20 via-rose-500/30 to-amber-500/20 hover:from-rose-500/30 hover:to-amber-500/30 border border-rose-400/40 text-white font-semibold text-xs uppercase tracking-widest transition-all duration-300 flex items-center gap-2.5 cursor-pointer hover:shadow-xl hover:shadow-rose-500/20 hover:scale-105"
          >
